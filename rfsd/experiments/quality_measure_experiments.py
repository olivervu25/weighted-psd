...
from kgof.density import UDFromCallable, IsotropicNormal
from kgof.kernel import KIMQ

from rfsd.rfsd import L1IMQFastKSD, LrSechFastKSD, KSD, PSD
...
def sgld_experiments(true_theta, log_prior, log_likelihood,
    ...
    num_sweeps = int(2 * num_samples / (N / batch_size))
...
def calculate_divergence_measures(samples, high_quality_samples, metrics,
    ...
    divergence_times = { name : np.zeros((len(eps_list), repetitions, divergence_reps)) for name in metrics }
    for i, eps in enumerate(eps_list):
        if verbose: print(i, eps)
        for j in range(repetitions):
            if verbose: print('  ', j+1, '/', repetitions)
            ...
            for k in range(divergence_reps):
                for name, dvs in divergence_values.items():
                    if 'KSD' not in name and 'PSD' not in name:
                        start_time = time.time()
                        dvs[i,j,k] = metrics[name].scaled_divergence(samples[i,j], J=J)
                        time_placeholder[name] = time.time() - start_time
                    elif k == 0:
                        start_time = time.time()
                        ...
                        dvs[i,j,k] = dvs[i,j,0]
                for name, dvt in divergence_times.items():
                        dvt[i,j,k] = time_placeholder[name]
                print("time_placeholder is ",time_placeholder)
    for name in divergence_values.keys():
        divergence_values[name] = divergence_values[name].reshape((len(eps_list), -1))
        divergence_times[name] = divergence_times[name].reshape((len(eps_list), -1))
    ...
    for name, dvs in divergence_values.items():
        if 'KSD' not in name and 'PSD' not in name:
            ...
            hq_divergence_times[name] = time.time() - tic
        else:
            tic = time.time()
            hq_divergence_values[name] = metrics[name].scaled_divergence(high_quality_samples[::5])
            hq_divergence_times[name] = time.time() - tic
    return divergence_values, hq_divergence_values, divergence_times, hq_divergence_times
...
def plot_sample_quality_results(divergence_values, hq_divergence_values,
                                divergence_times, hq_divergence_times,
    ...
    plt.figure()
    for i, (name, dvt) in enumerate(divergence_times.items()):
        c = color_dict[name] # palette[i]
        med_dvt = np.median(dvt, axis=1)
        hq_dv = hq_divergence_times[name]
        plt.plot(eps_list, med_dvt, c=c, label=name)#.decode())
        #plt.plot(eps_line, [hq_dv, hq_dv], '--', c=c)
        print('%-8s -' % name, eps_list[optimal_index])
    plt.xlabel(r'$\epsilon$')
    plt.ylabel('time')
    ...
    if J is not None:
        plt.title('$M$ = %d' % J)
    if save is not None:
        plt.savefig(save + '-no-legend_times.png', bbox_inches='tight')
    plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0., frameon=False)
    if save is not None:
        plt.savefig(save + '_times.png', bbox_inches='tight')
    if show:
        plt.show()
    ...
    for i, separation in enumerate(separations):
        ...
        if separation not in results:
            results[separation] = sgld_experiments(
                ...
                simulate_mixture_data, num_samples=10000,
                data_seed=5000+seed_base, sgld_seed=100+seed_base,
                verbose=verbose)
        ...
        for J in Js:
            ...
            if b_metrics_key not in results:
                ...
                results[b_metrics_key] = calculate_divergence_measures(
                    ...
                    J=J, verbose=verbose)
            divergence_values, hq_divergence_values, divergence_times, hq_divergence_times = results[b_metrics_key]
            if directory is not None:
                save_loc = directory + '/separation-%s-divergence-measures-J-%d' % (str(separation), J)
            plot_sample_quality_results(divergence_values, hq_divergence_values, divergence_times, hq_divergence_times,
