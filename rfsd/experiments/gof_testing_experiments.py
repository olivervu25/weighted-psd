...
from kgof.density import IsotropicNormal, GaussBernRBM, Normal
...
from rfsd.goftest import RFDH0SimCovDrawV, RFDGofTest, PolynomialSteinTest, PSDH0SimCovDraw, WPSDTest
...
### Gaussian vs Gaussian-Variance perturb experiment ###

def construct_gauss_mean_case(n, d):
    p = IsotropicNormal(np.zeros(d), 1)
    #perturb value
    e = 0.3
    mu = np.zeros(d)
    mu[0] += e 
    q = Normal(mu ,np.diag(np.ones(d))) 
    dat = q.get_datasource().sample(n, seed=None)
    return p, dat

def run_gauss_mean_goft_experiment(d=1, **kwargs):
    other_params = { 'd' : d}
    return run_goft_experiment(lambda n: construct_gauss_mean_case(n, d),
                               other_params, **kwargs)


def construct_gauss_perturb_case(n, d):
    p = IsotropicNormal(np.zeros(d), 1)
    #perturb value
    e = 0.7
    var = np.ones(d)
...
def run_gauss_perturb_goft_experiment(d=1, **kwargs):
    other_params = { 'd' : d}
    return run_goft_experiment(lambda n: construct_gauss_perturb_case(n, d),
                               other_params, **kwargs)
...
def single_gof_testing_round(test_names, case, gamma, test_alpha, J):
    ...
    l1_rfd_null_sim = RFDH0SimCovDrawV(n_draw=5000+d*500, seed=sim_seed, fromp=False)
    l1_rbm_rfd_null_sim = RFDH0SimCovDrawV(n_draw=5000, seed=sim_seed, fromp=False)
    l2_rfd_null_sim = RFDH0SimCovDrawV(n_draw=5000, seed=sim_seed, fromp=False)
    ...
    possible_tests = {    'RFSD (alpha)' : RFDGofTest(p, rfd_imq, null_sim=l1_rfd_null_sim, alpha=test_alpha),
                          'RFSD (RBM)' : RFDGofTest(p, rfd_imq_rbm, null_sim=l1_rbm_rfd_null_sim, alpha=l1_imq_rbm_test_alpha),
                          'RFSD' : RFDGofTest(p, rfd_imq, null_sim=l1_rfd_null_sim, alpha=l1_imq_test_alpha),
                          ...
                          'PSD r1' : PolynomialSteinTest(p, 1, alpha=test_alpha, seed=None, bootstrap = True),
                          'PSD r2' : PolynomialSteinTest(p, 2, alpha=test_alpha, seed=None, bootstrap = True),
                          'PSD r3' : PolynomialSteinTest(p, 3, alpha=test_alpha, seed=None, bootstrap = True),
                          'PSD r4': PolynomialSteinTest(p, 4, alpha=test_alpha, seed=None, bootstrap = True),
                          # wPSD (CE-optimised lambda) — varying polynomial order L
                          # split_ratio=0.7 -> 35/35/30 (Train1/Train2/Test)
                          'wPSD L=0'   : WPSDTest(p, Q=0,    alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=1'   : WPSDTest(p, Q=1,    alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=2'   : WPSDTest(p, Q=2,    alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=3'   : WPSDTest(p, Q=3,    alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=4'   : WPSDTest(p, Q=4,    alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=8'   : WPSDTest(p, Q=8,    alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=16'  : WPSDTest(p, Q=16,   alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=32'  : WPSDTest(p, Q=32,   alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=64'  : WPSDTest(p, Q=64,   alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=128' : WPSDTest(p, Q=128,  alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=256' : WPSDTest(p, Q=256,  alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=512' : WPSDTest(p, Q=512,  alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          'wPSD L=1024': WPSDTest(p, Q=1024, alpha=test_alpha, method='tpsd', split_ratio=0.7),
                          # PSD (uniform lambda) — varying polynomial order L
                          # split_ratio=0.7 -> 70/30 (Train/Test)
                          'PSD L=0'   : WPSDTest(p, Q=0,    alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=1'   : WPSDTest(p, Q=1,    alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=2'   : WPSDTest(p, Q=2,    alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=3'   : WPSDTest(p, Q=3,    alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=4'   : WPSDTest(p, Q=4,    alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=8'   : WPSDTest(p, Q=8,    alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=16'  : WPSDTest(p, Q=16,   alpha=test_alpha, method='psd', split_ratio=0.7),
                          'PSD L=32'  : WPSDTest(p, Q=32,   alpha=test_alpha, method='psd', split_ratio=0.7),
    }
    if 'Gauss FSSD-opt' in test_names:
...
def print_experiment_results(results):
    ...
    for test_name, test_rejects in rejects.items():
        ...
        print('*', test_name, r'reject rate (95% confidence interval): ',
              ...
              (round(l, 3), round(u, 3)))
        res = stats.binomtest(test_rejects, rounds, .05)
        print(' ' * len(test_name), '  p-value for p = .05:',
              res.pvalue if res != None else 0)
