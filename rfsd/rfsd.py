...
class KSD(Divergence):
    ...
    def divergence(self, X, Y=None, **kwargs):
        ...
        # print("KSD took: ",time.time() - start_time)
        return ksd
...
class PSD(Divergence): 
    def __init__(self, polyorder, p):
    ...
    def divergence(self, X, Y=None, **kwargs):
        ...
        # print("polyorder is ",polyorder," and Z has ",np.size(Z,1)," columns")
        # print("PSD",polyorder," took: ",time.time() - start_time)
        Zbar = np.mean(Z, axis = 0)
        psd = np.linalg.norm(Zbar, 2)
        return psd
...
class RandomFeatureDivergence(Divergence):
    ...
    def divergence(self, X, Y=None, J=10, Z=None, reuse_features=False, ordering=None,
        ...
        divergence = self.compute_divergence(mean_feature_tensor,
                                             ordering=ordering)
        #print("Approximate took: ",time.time() - start_time)
    ...
    def _generate_imq_f1(self, c, beta, d, gamma, nu_type, target_df):
        ...
        if nu_type == 'mean':
            ...
            def nu_log_density (x, mean, X):
                ...
                wts = np.linalg.norm(grad_log_p, axis=1)
                lls += np.log(wts)
                return special.logsumexp(lls, axis=1) - np.log(np.sum(wts))
